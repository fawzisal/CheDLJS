...
export function liquid_jet_pump_ancillary({rhop, rhos, Kp, Ks, d_nozzle=null, d_mixing=null, Qp=null, Qs=null, P1=null, P2=null}) {
    let unknowns = sum([d_nozzle, d_mixing, Qs, Qp, P1, P2].map((i) => i === null));
    if( unknowns > 1 ) {
    ...
    if( d_nozzle !== null ) {
        let A_nozzle = Math.PI/4*d_nozzle*d_nozzle;
        if( d_mixing !== null ) {
            let A_mixing = Math.PI/4*d_mixing*d_mixing;
            let R = A_nozzle/A_mixing;
        }
    }
    if( P1 === null ) {
...
const { uniform, seed } = require( './random' );
    ...
    function obj_err(val) {
        ...
        // variables
        for( let [ i, v ] of _pyjs.listZip(unknown_vars, val) ) {
            vals[i] = abs(float(v));
        }
    ...
    //                 }
    //                 for( let [ u, v ] of _pyjs.listZip(unknown_vars, res[0].tolist()) ) {
    //                     vals[u] = abs(v);
    //                 }
    //                 return vals;
    //             }
    ...
    //                         throw new Error( 'ValueError','Could not solve' );
    //                     }
    //                     for( let [ u, v ] of _pyjs.listZip(unknown_vars, res['x'].tolist()) ) {
    //                         vals[u] = abs(v);
    //                     }
    //                     return vals;
    ...
    //             }
    //             for( let _ of range(int(max_variations/8)) ) {
    //                 for( let idx of [0, 1] ) {
...
export function vacuum_air_leakage_Ryans_Croll({V, P, P_atm=101325.0}) {
    ...
    if( P_vacuum < 10 ) {
        let air_leakage = 0.026*P_vacuum**0.34*V**0.6;
...
export function vacuum_air_leakage_HEI2633({V, P, P_atm=101325.0}) {
    ...
    if( V < 10 ) { let V = 10.0; }
    let logV = Math.log(V);
    if( P_vacuum <= 1 ) { let [c0, c1] = [0.6667235169997174, -3.71246576520232]; } 
    else if( P_vacuum <= 3 ) { let [c0, c1] = [0.664489357445796, -3.0147277548691274]; } 
    else if( P_vacuum <= 20 ) { let [c0, c1] = [0.6656780453394583, -2.34007321331419]; } 
    else if( P_vacuum <= 90 ) { let [c0, c1] = [0.663080000739313, -1.9278288516732665]; } 
